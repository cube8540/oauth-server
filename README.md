...
# HTTP API
OAuth2 토큰 발급에 관련되어 있지 않은 API 입니다. 아래의 API를 이용하여 OAuth2 클라이언트 추가 및 수정, 유저 등록 등을 할 수 있습니다. 아래는 현재 구현되어 있는 API 리스트 입니다.
## 계정 HTTP API
|              Endpoint             | Method |                설명                |
| --------------------------------- | :------: | ----------------------------------------- |
| /api/accounts                     |  POST  | 새 유저를 등록 합니다.                    |
...
| /api/authorities/{code}          |   GET  | 권한의 상세 정보를 검색 합니다.              |
...
| /api/scopes/{id}                  |  PUT   | 스코프의 정보를 변경 합니다.              |
| /api/scopes/{id}                  | DELETE | 스코프를 삭제 합니다.                     |