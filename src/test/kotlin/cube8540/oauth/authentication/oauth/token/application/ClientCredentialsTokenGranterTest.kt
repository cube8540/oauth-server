...
class ClientCredentialsTokenGranterTest {
    ...
    private val granter = ClientCredentialsTokenGranter(tokenIdGenerator, repository)
    ...
    @Test
    fun `create access token when request scope is not allowed`() {
        ...
        every { validator.validateScopes(clientDetails, setOf("request-scope-1", "request-scope-2", "request-scope-3")) } returns false
    ...
    fun `create access token when request scope is null`() {
        ...
        assertThat(result.scopes).isEqualTo(setOf(AuthorityCode("client-scope-1"), AuthorityCode("client-scope-2"), AuthorityCode("client-scope-3")))
    }
    ...
    @Test
    fun `create access token when refresh token set false`() {
        val clientDetails: OAuth2ClientDetails = mockk(relaxed = true) {
            every { clientId } returns "clientId"
            every { scopes } returns setOf("client-scope-1", "client-scope-2", "client-scope-3")
        }
        ...
        granter.allowedRefreshToken = false
        every { validator.validateScopes(clientDetails, setOf("request-scope-1", "request-scope-2", "request-scope-3")) } returns true
    ...
    @Test
    fun `create access token when refresh token set true and refresh token id generator set null`() {
        val clientDetails: OAuth2ClientDetails = mockk(relaxed = true) {
            every { clientId } returns "clientId"
            every { scopes } returns setOf("client-scope-1", "client-scope-2", "client-scope-3")
        }
        ...
        granter.allowedRefreshToken = true
    ...
    @Test
    fun `create access token when refresh token set true and refresh token id generator is set`() {
        val clientDetails: OAuth2ClientDetails = mockk(relaxed = true) {
            every { clientId } returns "clientId"
            every { scopes } returns setOf("client-scope-1", "client-scope-2", "client-scope-3")
        }
