...
class ScopeInvalidException(code: String, errors: Collection<ValidationError>): ServiceInvalidException(code, errors) {
    companion object {
        @JvmStatic fun instance(errors: List<ValidationError>): ScopeInvalidException =
...
class ScopeNotFoundException(code: String, description: String): ServiceException(code, description) {
    companion object {
        @JvmStatic fun instance(description: String) = ScopeNotFoundException(ErrorCodes.NOT_FOUND, description)
    }
}

class ScopeRegisterException(code: String, description: String): ServiceException(code, description) {
    companion object {
        @JvmStatic fun existsIdentifier(description: String) = ScopeRegisterException(ErrorCodes.EXISTS_IDENTIFIER, description)
    }
}